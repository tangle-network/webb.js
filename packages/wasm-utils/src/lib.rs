...
impl PedersonNoteGenerator {
  ...
  pub fn leaf_of(&self, note: &Note) -> Uint8Array {
    let pre_image:[u8;62] = [note.nullifier.as_bytes(), note.r.as_bytes()].concat();
    let pre_image_hash = pedersen_hash(preImage).unwrap();
    let leaf = Scalar::from_bits(pre_image_hash);
