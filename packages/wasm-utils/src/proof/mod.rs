...
use arkworks_gadgets::setup::mixer::{
	prove_groth16_x17, prove_groth16_x5, setup_arbitrary_data, setup_leaf_x17, setup_leaf_x5, setup_random_groth16_x17,
	setup_random_groth16_x5, Circuit_x17, Circuit_x5, Leaf_x5,
};
			...
			Curve::Bn254 => {
				...
				match "5" {
					...
					"17" => {
						...
						let arbitrary_input = setup_arbitrary_data::<FrBn254>(recipient, relayer, fee, refund);
						let (_leaf_private, leaf, _nullifier_hash) = setup_leaf_x17::<_, FrBn254>(&params17, &mut rng);
						let mut leaves_new: Vec<FrBn254> = proof_input
							.leaves
							.to_vec()
							.into_iter()
			...
			Curve::Bls381 => {
				...
				match "5" {
					...
					"5" => {
						...
						let (_leaf_private, _leaf, _nullifier_hash) = setup_leaf_x5::<_, FrBls381>(&params5, &mut rng);
