...
impl FromStr for Note {
  ...
  fn from_str(s: &str) -> Result<Self, Self::Err> {
    ...
    let nullifier: Vec<u8> = hex::decode(&note_val[64..])
      .map(|v| v.try_into())
      .map_err(|_| OpStatusCode::InvalidHexLength)?
      .map_err(|_| OpStatusCode::HexParsingFailed)?;
    ...
    let denomination = parts[9].to_string();
    let chain = parts[10].to_string();
    let amount = parts[11].to_string();
    Ok(Note {
