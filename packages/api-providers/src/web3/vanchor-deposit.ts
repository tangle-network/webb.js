...
export class Web3VAnchorDeposit extends VAnchorDeposit<WebbWeb3Provider, DepositPayload> {
  ...
  async generateBridgeNote (anchorId: string | number, destination: number, amount: number, wrappableAssetAddress?: string): Promise<DepositPayload> {
    ...
    const noteInput: NoteGenInput = {
      ...
      protocol: 'vanchor',
      secrets: `${toFixedHex(destination, 6).substring(2)}:${BigNumber.from(depositOutputUtxo.amount).toHexString()}:${depositOutputUtxo.keypair.pubkey.toHexString()}:${BigNumber.from(depositOutputUtxo.blinding).toHexString()}`,
  ...
  async deposit (depositPayload: DepositPayload): Promise<void> {
    ...
    try {
      ...
      if (depositPayload.params[2]) {
        const requiredApproval = await vanchorWrapper.isWrappableTokenApprovalRequired(depositPayload.params[2], amount);
