...
import { AbiCoder, BigNumberish, keccak256 } from 'ethers';
...
export function getVAnchorExtDataHash (
  ...
  return BigInt(hash) % BigInt(FIELD_SIZE.toString());
}
...
export function generateVariableWitnessInput (
  ...
  const input = {
    ...
    outputCommitment: outputs.map((x) => BigInt(u8aToHex(x.commitment)).toString()),
    publicAmount: (BigInt(extAmount.toString()) - BigInt(fee.toString()) + BigInt(FIELD_SIZE.toString()) % BigInt(FIELD_SIZE.toString())).toString(),
    extDataHash: extDataHash.toString(),

    // data for 2 transaction inputs
    inAmount: inputs.map((x) => x.amount.toString()),
    inPrivateKey: inputs.map((x) => '0x' + x.secret_key),
    inBlinding: inputs.map((x) => BigInt('0x' + x.blinding).toString()),
    ...
    outPubkey: outputs.map((x) => BigInt(x.getKeypair().getPubKey()).toString()),
    outBlinding: outputs.map((x) => BigInt('0x' + x.blinding).toString())
  };
